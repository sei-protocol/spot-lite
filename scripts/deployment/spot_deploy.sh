...
seid=~/go/bin/seid
code=$(printf $password | $seid tx wasm store $contract -y --from=$keyname --chain-id=sei-devnet-3 --node=http://3.73.138.242:26657 --gas=10000000 --fees=10000000usei --broadcast-mode=block | grep -A 1 "code_id" | sed -n 's/.*value: "//p' | sed -n 's/"//p')
admin_addr=$(printf $password |$seid keys show $keyname | grep -A 1 "address" | sed -n 's/.*address: //p')

addr=$(printf $password |$seid tx wasm instantiate $code "{}" --from $keyname --broadcast-mode=block --node=http://3.73.138.242:26657 --label "spot" --admin $admin_addr --chain-id sei-devnet-3 --gas=30000000 --fees=300000usei -y | grep -A 1 -m 1 "key: _contract_address" | sed -n 's/.*value: //p' | xargs)
...
printf $password |$seid tx dex register-contract $addr $code true true 100000000 -y --from=$keyname --chain-id=sei-devnet-3 --fees=10000000usei --gas=10000000 --broadcast-mode=block --node=http://3.73.138.242:26657
